<h1 align="center">Hi 👋, I'm Vishal Ajai Rosen V</h1>

<h3 align="center">
AI/ML Engineer • LLM Systems • Agentic AI • RAG • Building production-ready AI applications
</h3>

---

## 🚀 About Me

- 🔭 Currently working on **Agentic RAG & Multi-Agent Systems.**
- 🌱 Learning **GraphRAG, MCP & LLM Optimization.**
- 💬 Ask me about **LLMs, RAG, LangGraph, LangChain, Qdrant, Airflow & Docker**
- 📫 **vishalajairosen.works@gmail.com**

---

## 🛠️ Tech Stack

**Languages:** Python • SQL • R

**GenAI:** RAG • GraphRAG • Agentic AI • LangChain • LangGraph • LoRA • Prompt Engineering • MCP

**ML:** PyTorch • TensorFlow • Scikit-Learn • BERTopic • Sentence Transformers

**Vector Search:** Qdrant • Hybrid Search • HNSW • Cross-Encoder

**Data & MLOps:** Docker • Airflow • MLflow • DVC • Azure • PySpark • LangSmith

**Databases:** MySQL • MongoDB • Neo4j • PostgreSQL

---

## 🚀 Featured Projects

- 🤖 Production-scale **Agentic RAG** System
- ⚽ **Football GraphRAG** Chatbot
- 🧠 **GPT from Scratch** (PyTorch)
- 🤝 **Multi-Agent System** using LangGraph & MCP
- 📦 **Swiggy Scraper** Python Package (PyPI)

---

## 🌐 Connect

- 💻 GitHub: https://github.com/ajairosen
- 💼 LinkedIn: https://linkedin.com/in/vishal-ajai-rosen-v-3b6648312
- 📧 Email: vishalajairosen.works@gmail.com

---

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=ajairosen&show_icons=true&theme=tokyonight" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=ajairosen&theme=tokyonight" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ajairosen&layout=compact&theme=tokyonight" />
</p>

<p align="center">
<i>"Building AI systems that retrieve, reason, and take action."</i>
</p>
